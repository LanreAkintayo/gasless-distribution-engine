...
contract GaslessDistroV1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // ---------------- Initializer ----------------
    function initialize(address _tokenAddress, address initialOwner, address _voucherSigner)
        public
        initializer
    {
    ...
    function setAmountToClaim(uint256 newAmount) external onlyOwner {
        amountToClaim = newAmount;
    }

    function _authorizeUpgrade(address newImplementation) internal override onlyOwner {}

    // ---------------- View Functions ----------------
    function hasUsedVoucher(uint256 nonce) external view returns (bool) {
